function [features,projectedTrials,Filter] = extractCSP(EEGSignals, CSPMatrix, nbFilterPairs)
...
% [Nt * (nbFilterPairs*2] without label
%
%initializations
...
projectedTrials=zeros(nbTrials,2*nbFilterPairs,size(EEGSignals.x,1));
%extracting the CSP features from each trial
for t=1:nbTrials    
    %projecting the data onto the CSP filters    
    projectedTrial = Filter * EEGSignals.x(:,:,t)';    
    projectedTrials(t,:,:)=projectedTrial;
    %generating the features as the log variance of the projectedsignals

    variances = var(projectedTrial,0,2);
    % 修改
    variances= log(variances./sum(variances));
    features(t,:)=variances;
